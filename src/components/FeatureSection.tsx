...
type FeatureKey = 'editor' | 'color' | 'search' | 'backup'
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className={`flex flex-col ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-12`}>
          ...
          <div className="flex-1 max-w-md">
            <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-[#5A7FEF]/10 dark:bg-[#7A9FFF]/10 mb-5">
              <Icon size={20} className="text-[#5A7FEF] dark:text-[#7A9FFF]" />
            </div>
            <h3 className="text-2xl font-bold text-[#1d1d1f] dark:text-[#e8e8ed] mb-2">{feature.title}</h3>
            <p className="text-sm font-medium text-[#5A7FEF] dark:text-[#7A9FFF] mb-4">{feature.subtitle}</p>
